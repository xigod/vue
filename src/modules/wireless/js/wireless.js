define(function (require, exports, module) {
	...
	function changeWifiRelayType(type) {
		$("#wifiScheduleWrap, #wifiParamWrap, #wpsWrap").addClass("none");

		switch (type) {
		case "disabled":
			$("#wifiScheduleWrap, #wifiParamWrap,#wpsWrap").removeClass("none");
			break;
		case "wisp":
	...
	function WifiTimeModule() {
		...
		this.checkData = function () {
			//这两种模式下不需要传该模块值;
			if (pageModule.data.wifiTime.wifiRelayType == "wisp" || pageModule.data.wifiTime.wifiRelayType == "client+ap") {
				return;
			}
	...
	function WifiWPSModule() {
		...
		this.getSubmitData = function () {
			//这两种模式下不需要传该模块值;
			if (pageModule.data.wifiTime.wifiRelayType == "ap" || pageModule.data.wifiTime.wifiRelayType == "client+ap") {
				return "";
			}
		...
		function changeWpsEn() {
			if ($("input[name='wpsEn']").length > 0 && $("input[name='wpsEn']")[0].checked) {
				...
				// $("#wifiSecurityMode").val(pageModule.data.wifiBasicCfg.wifiSecurityMode).attr("disabled", true);
				$("#wifiSecurityMode").attr("disabled", true);
				// $("#wifiPwd").val(pageModule.data.wifiBasicCfg.wifiPwd).attr("disabled", true);
	...
	function WifiAdvCfgModule() {
		...
		function createChannel(max) {
			...
			for (i = 1; i <= max; i++) {
				...
				str += '<option value="' + i + '">' + _("Channel ") + i + "</option>";
			}
