define(function (require, exports, module) {
	...
	function AttachedModule() {
		...
		function createOnlineList(obj) {
			...
			for (k = 0; k < macFilterListLen; k++) {
				...
				if (obj.macFilterList[k].remark != "") {
					...
					hostname = obj.macFilterList[k].hostname;
				}

				$("#qosListAccess .addListTag").find(".deviceName div").text(hostname);
				$("#qosListAccess .addListTag").find(".deviceName").attr("data-mark", obj.macFilterList[k].hostname);
		...
		function getBlockedList() {
			...
			for (i = 0; i < length; i++) {
				var tmpObj = {};
				tmpObj.hostname = $listTable.eq(i).find("td[data-mark]").attr("data-mark");
				if (tmpObj.hostname == $listTable.eq(i).children().eq(0).text()) {
					tmpObj.remark = "";
				} else {
					tmpObj.remark = $listTable.eq(i).find("td[data-mark] div").text(); //device name
				}
