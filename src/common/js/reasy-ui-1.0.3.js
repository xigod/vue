...
(function () {
	...
	var Textboxs = {
		...
		create: function (elem, type, defVal) {
			...
			defVal = defVal || '';
			type = type || 'ip';
			classStr = type === 'ip-mini' ? 'form-control input-mini' : 'form-control';
		...
		addCapTip: function (newField, pasElem) {
			...
			$newField.on("keyup", function (e) {
				...
				if (!this.capDetected) {
					...
					massageElm.id = msgId;
					massageElm.innerHTML = _("Capital characters are entered.");
					if (pasElem) {
		...
		addPlaceholder: function (elem, placeholderText) {
			...
			function createPlaceholderElem(elem) {
				...
				ret.innerHTML = '<span class="placeholder-text" style="width:' +
					(elem.offsetWidth || 300) + 'px;line-height:' +
...
(function (document) {
	...
	var Inputselect = {
		...
		create: function (elem, obj) {
			...
			inputBoxStr = '<input class="form-control input-box' + '" type="text" ' + inputAble + ' value="' + obj.initVal[0] + '">' +
				'<input type="hidden" value="' + obj.initVal[1] + '">';
			dropBtnStr = '<div class="btn-group"><button type="button"' + toggleAble + ' class="dropdown-toggle toggle btn btn-sm"><span class="icon-caret"></span></button></div>';
			ulStr = '<div class="dropdown"><ul class="dropdown-menu">' + liContent + '</ul></div>';
...
(function (window, document) {
	...
	var utils = {
			...
			check: function (eventType) {
				...
				//不是隐藏并且没有disabled
				if (str && (!$.isHidden(this) && !$.isDisabled(this))) {
			...
			blur: function (e) {
				...
				if (data && typeof (data.type) !== "undefined") {
					if (data.type === "num" && this.value != "") {
						if (!$(this).parent().hasClass("has-error") && !isNaN(this.value) && data.args && data.args.length > 0) {
	...
	valid = {
		...
		ascii: function (str, min, max) {
			...
			if (min || max) {
				return $.validate.valid.len(str, min, max);
			}
		},
		...
		/*
			@modify by pengjuanli
			@支持"英文字母"、"数字"、"-"、".", 长度小于67字符。例如：xxxx.xxx.xx每个xxx块长度不能大于22
		 */
		/*domain: function (str) {
			if (/^[-\.0-9a-z]+$/ig.test(str)) {
				if (/[0-9a-z-].{1, }+/ig.test(str) || str == "localhost") {
...
(function (document) {
	...
	var Dropdown = {
		create: function (elem, input) {
			var id = elem.id,
				str = "";
			str = '<label for="' + id + '" class="select-btn"><div class="select-caret"></div></label>';
