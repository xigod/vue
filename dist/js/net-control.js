	...
	function getBlackLength() {
		...
		if (length == 1 && $listTable.eq(0).children().length < 2) {
			...
			for (i = 0; i < length; i++) {
				...
				//判断是否关闭 或者上传或下载限制为0
				if ($tr.find(".internet-ctl").children().hasClass("icon-toggle-off") || $tr.find(".qosListUpLimitTd").find(".input-append")[0].val() == "0" || $tr.find(".qosListDownLimitTd").find(".input-append")[0].val() == "0") {
					index++;
				}
			}
		}
	...
	function AttachedModule() {
		...
		function createOnlineList(obj) {
			...
			for (i = 0; i < onlineListlen; i++) {
				...
				for (prop in obj.onlineList[i]) {
					...
					case "qosListHostname":
						...
						str += '</div>';
						str += '<div class="col-xs-1 row"> <span class="ico-small icon-edit"></span> </div>';
			...
			for (k = 0; k < macFilterListLen; k++) {
				...
				if (obj.macFilter.macFilterList[k].remark != "") {
					...
					deviceName = obj.macFilter.macFilterList[k].hostname;
				}

				$("#qosListAccess .addListTag").find(".deviceName div").text(deviceName);
				$("#qosListAccess .addListTag").find(".deviceName").attr("data-mark", obj.macFilter.macFilterList[k].hostname);
			...
			for (i = 0; i < getOnlineLen; i++) {
				...
				for (j = 0; j < onlineTbodyLen; j++) {
					...
					if (oldMac == newMac) { //存在
						rowData[j] = {};
						//在线或本机
						if (!$onlineTbodyList.eq(j).children().find('.internet-ctl').children().hasClass("icon-toggle-off")) { //当前为允许时
		...
		function getAttacheList() {
			...
			for (i = 0; i < length; i++) {
				...
				$tr = $listTable.eq(i);
				//允许
				internetAccess = $tr.find(".internet-ctl").children().hasClass("icon-toggle-on");
				//本机
		...
		function getBlockedList() {
			...
			for (i = 0; i < length; i++) {
				...
				//拒绝访问
				tmpObj.hostname = $listTable.eq(i).children().eq(0).attr("data-mark");

				if (tmpObj.hostname == $listTable.eq(i).children().eq(0).text()) {
					tmpObj.remark = "";
				} else {
					tmpObj.remark = $listTable.eq(i).children().eq(0).text(); //device name
				}
